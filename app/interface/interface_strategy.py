...
class InterfaceStrategy(ABC):
    @abstractmethod
    def run(self, sender, receiver, note_service, user_service, talhao_service=None) -> None:
        ...


class GuiInterfaceStrategy(InterfaceStrategy):
    def run(self, sender, receiver, note_service, user_service, talhao_service=None) -> None:
        app = NotesAppGUI(sender, receiver, note_service, user_service, talhao_service)
        app.run()
